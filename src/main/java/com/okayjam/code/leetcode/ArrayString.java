...
//        System.out.println(new ArrayString().findSubsequences(new int[]{4,6,7,7}));
    ...
    public int findIntegers(int n) {
        // 1. 预处理 dp 数组，dp[i] 表示长度为 i 的二进制字符串中无连续 1 的合法方案数
        // * 如果最高位填 0，则剩下 i-1 位可以任意合法填报，方案数为 dp[i-1]
        // * 如果最高位填 1，为避免连续 1，次高位必须填 0，剩下 i-2 位可以任意合法填报，方案数为 dp[i-2]
        int[] dp = new int[31];
        dp[0] = 1;
        dp[1] = 2;
        for (int i = 2; i < 31; i++) {
        ...
        int res = 0;
        int preBit = 0; // 记录前一位的二进制值

        // 2. 从最高位（第 29 位，因为 10^9 < 2^30）向低位遍历
        for (int i = 29; i >= 0; i--) {
            int val = 1 << i;
            if ((n & val) != 0) {
                // 当前位i是 1 (i是从0算的，所以后续不是累加dp[i+1])
                // 如果我们假设填0，累加把当前位填 0 时的合法方案数dp[i]
                // 随后，我们保留这一位填 1 的状态，继续考察更低位。
                res += dp[i];
                // 如果出现连续的 1，说明后面的路径均非法，提前结束遍历(因为我们默认这位填1了，如果前面一位也填1那就会导致后续的所有结果都不对)
                if (preBit == 1) {
                    break;
                }
                preBit = 1;
            ...
            // 如果成功遍历到最后一位且无连续 1，说明 n 本身也是合法的，答案 +1
            if (i == 0) {
                res += 1;
            }
        }
